...
- `openbee service install` on Windows no longer falsely reports `service did not enter running state` on non-English locales. The Task Scheduler state probe previously parsed `schtasks /Query /V /FO LIST` text (`Status: Running` / `Ready`), both of which are localized — Chinese Windows surfaces them as `状态: 正在运行` / `就绪`, so the run-state always stayed Unknown even though the daemon had started. The probe now reads `Get-ScheduledTask .State` via PowerShell (English enum across locales) and falls back to a `tasklist` lookup for `openbee.exe` when PowerShell is unavailable.
