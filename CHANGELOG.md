...
### Removed
- Drop Windows support for `openbee service install/uninstall/start/stop/status`. The Task Scheduler (`schtasks`) backend, its embedded `schtask.xml.tmpl` template, and the Windows-specific manager are removed; on Windows the subcommands now return `openbee service is not supported on windows`. The `openbee` binary itself is still built and supported on Windows for all other commands (e.g. `openbee server`, `openbee ctl …`); users who want autostart on Windows should register a Task Scheduler entry manually.
...
- `openbee service install --working-dir` flag to override the service working directory; defaults to `~/.openbee`. The launchd plist and systemd unit now set a working directory explicitly, so relative paths in `config.yaml` (e.g. the default `./data/openbee.db`) no longer resolve to the OS default cwd (which is `/` for launchd and produced `mkdir data: read-only file system`).
...
- `openbee service install` on Linux now derives the systemd unit's `PATH` from the run-as user's login shell (`runuser -u <user> -- /bin/sh -lc 'printf %s "$PATH"'`) instead of the installer's `PATH`. Previously, `sudo openbee service install` baked root's `PATH` into the unit; if it pointed at `/root/.nvm/.../bin/node`, chat-time invocation of `claude` failed with `/usr/bin/env: 'node': Permission denied` because the daemon user could not traverse `/root`. The install command also distinguishes "node missing" from "node not executable by run-as user" and prints a dedicated warning for the latter.
- `openbee service install` on Linux now wraps `ExecStart` in `/bin/bash -lc 'exec ...'` and stops baking `Environment=PATH=` / `Environment=HOME=` into the systemd unit. The daemon now inherits the run-as user's live login-shell environment (nvm, conda activate scripts, custom `PATH`/`NODE_OPTIONS`) at start time, so changes to the user's profile (e.g. switching nvm versions, installing Node.js after the service was installed) take effect on the next `systemctl restart openbee` without requiring `openbee service install --force`. The install-time node-availability probe was migrated to the same login-shell path (`runuser -l <user> -c …`) so its warning matches what the daemon will actually see at runtime.
- `openbee service install` on Linux now uses `/bin/bash -ilc` (interactive + login) for `ExecStart` and the install-time PATH/node probes (`runuser -l <user> -c 'bash -ic …'`). The previous `bash -lc` form was non-interactive, so it did not source `~/.bashrc` — the default location where nvm's `nvm.sh` is registered on most distros. When the run-as user's nvm setup lived only in `~/.bashrc`, the daemon's spawned `claude`/`codex` Node.js CLIs failed with `/usr/bin/env: 'node': No such file or directory` even though `node` was reachable in an interactive shell. The interactive-login shell sources both the login profile and `~/.bashrc`, restoring node visibility.
...
- Remove the Kimi engine and all related code, config, and references.
