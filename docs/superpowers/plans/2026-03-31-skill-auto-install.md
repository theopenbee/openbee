# Skill Auto-Install Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** After `openbee config` completes Claude executable configuration, automatically install/update the built-in `bee` and `worker` skills into `~/.claude/skills/` using SHA-256 hash comparison to detect changes.

**Architecture:** New `internal/skillinstall` package holds the two SKILL.md files as string constants and exposes `InstallSkills(baseDir string) ([]SkillResult, error)`. The `config` command calls a thin `installBuiltinSkills()` wrapper after `configureClaudeProvider` succeeds; failures are non-fatal (warning printed, config continues). Three i18n keys are added for user-visible output.

**Tech Stack:** Go standard library only — `crypto/sha256`, `os`, `path/filepath`.
...
|---|---|---|
| `internal/skillinstall/skills_data.go` | Create | String constants for bee and worker SKILL.md content |
| `internal/skillinstall/install.go` | Create | `InstallSkills()` function and `SkillResult` type |
| `internal/skillinstall/install_test.go` | Create | Unit tests using `t.TempDir()` |
| `internal/i18n/messages.go` | Modify | Add `SkillInstalled`, `SkillUpdated`, `SkillsInstallWarning` to `ConfigOutput` |
| `internal/i18n/locales/en.yaml` | Modify | English strings for the three new keys |
| `internal/i18n/locales/zh.yaml` | Modify | Chinese strings for the three new keys |
| `cmd/openbee/config.go` | Modify | Add `installBuiltinSkills()` helper and call it after `configureClaudeProvider` |

---

### Task 1: Create `skills_data.go` with embedded skill content

**Files:**
- Create: `internal/skillinstall/skills_data.go`

- [ ] **Step 1: Create `internal/skillinstall/skills_data.go`**
...
- [ ] **Step 2: Verify the file compiles (no logic yet, just data)**

```bash
cd /path/to/openbee-worktree
go build ./internal/skillinstall/...
```

Expected: no errors (package compiles with just data declarations)

- [ ] **Step 3: Commit**

```bash
git add internal/skillinstall/skills_data.go
git commit -m "feat(skillinstall): add embedded bee and worker SKILL.md content"
```
...
**Files:**
- Create: `internal/skillinstall/install_test.go`

- [ ] **Step 1: Create `internal/skillinstall/install_test.go`**
...
```bash
cd /path/to/openbee-worktree
go test ./internal/skillinstall/... -v
```

Expected: FAIL — `InstallSkills` and `SkillResult` are not defined yet.

- [ ] **Step 3: Commit**

```bash
git add internal/skillinstall/install_test.go
git commit -m "test(skillinstall): add failing tests for InstallSkills"
```

---

### Task 3: Implement `InstallSkills`

**Files:**
- Create: `internal/skillinstall/install.go`

- [ ] **Step 1: Create `internal/skillinstall/install.go`**
...
```bash
git add internal/skillinstall/install.go
git commit -m "feat(skillinstall): implement InstallSkills with hash-based update detection"
```
...
**Files:**
- Modify: `internal/i18n/messages.go` (lines ~199–222, `ConfigOutput` struct)
- Modify: `internal/i18n/locales/en.yaml` (inside `config:` block, after `claude_manual_entry`)
- Modify: `internal/i18n/locales/zh.yaml` (same location)

- [ ] **Step 1: Add fields to `ConfigOutput` in `internal/i18n/messages.go`**

In `ConfigOutput` struct (around line 221), add after `ClaudeManualEntry`:
...
- [ ] **Step 2: Add English strings to `internal/i18n/locales/en.yaml`**

In the `config:` block, after the `claude_manual_entry` line, add:

```yaml
    skill_installed: "skill %s installed"
    skill_updated: "skill %s updated"
    skills_install_warning: "skills install warning (skipped): %v"
```

- [ ] **Step 3: Add Chinese strings to `internal/i18n/locales/zh.yaml`**

In the `config:` block, after the `claude_manual_entry` line, add:

```yaml
    skill_installed: "skill %s 已安装"
    skill_updated: "skill %s 已更新"
    skills_install_warning: "安装 skills 时出错（已跳过）: %v"
```

- [ ] **Step 4: Verify i18n still loads without errors**
...
git commit -m "feat(i18n): add skill install/update output messages"
```

---

### Task 5: Integrate into `config` command

**Files:**
- Modify: `cmd/openbee/config.go`

- [ ] **Step 1: Add `skillinstall` import to `cmd/openbee/config.go`**

In the `import` block of `cmd/openbee/config.go`, add:

```go
"github.com/theopenbee/openbee/internal/skillinstall"
```

- [ ] **Step 2: Add `installBuiltinSkills()` helper function to `cmd/openbee/config.go`**

Add this function at the end of `config.go` (before or after `handleSurveyErr`):
...
- [ ] **Step 3: Call `installBuiltinSkills()` in `runConfig()` after `configureClaudeProvider`**

In `runConfig()`, locate these lines (around line 194–196):
...
git commit -m "feat(config): install built-in bee and worker skills after claude config"
```
...
**Spec coverage check:**
- ✅ Install `bee` and `worker` skills to `~/.claude/skills/` — Task 1 + Task 3
- ✅ Triggered after `openbee config` Claude config step — Task 5
- ✅ Content embedded in binary — Task 1 (`skills_data.go`)
- ✅ Hash comparison (installed / updated / up-to-date) — Task 3
- ✅ Non-fatal error handling — Task 5 (`installBuiltinSkills`)
- ✅ i18n keys for output — Task 4
- ✅ Unit tests (4 cases) — Task 2 + Task 3
- ✅ `baseDir` override for tests — Task 3 (`InstallSkills` signature)

**No placeholders, no TODOs, no missing types.** `SkillResult` defined in Task 3 and used in Task 5. `beeSkillMD` / `workerSkillMD` defined in Task 1 and referenced in Task 2's test (`string(got) != beeSkillMD`).
