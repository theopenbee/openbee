...
- Create `internal/bridge/engines.go`: engine adapter construction and registration imports.
...
- [ ] **Step 3: Add engine construction helper**
...
```bash
git add internal/bridge/engines.go internal/bridge/service.go internal/bridge/service_test.go
...
func buildBridge(cfg config.BeeConfig, engineCfg *enginecfg.Store, envSvc *env.Service, sysStore *store.SystemConfigStore) (bridge.Bridge, error) {
	engines, err := bridge.BuildEngines(cfg.RPCBaseURL, cfg.Engines.IsEnabled, cfg.EngineConfigRawFor)
	if err != nil {
		return nil, err
	}
