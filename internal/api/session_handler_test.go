...
func TestExecutionsList_IncludesTokenStats(t *testing.T) {
	...
	if _, err := es.Create(store.ExecutionCreate{WorkerID: "worker-1", TriggerInput: "hello", SessionID: "session-abc", Engine: "claude"}); err != nil {
...
func TestExecutionsList_NoTokenStats_WhenNoneExist(t *testing.T) {
	...
	if _, err := es.Create(store.ExecutionCreate{WorkerID: "worker-1", TriggerInput: "hello", SessionID: "session-xyz", Engine: "claude"}); err != nil {
		t.Fatalf("Create execution: %v", err)
	}
