...
// ExecuteWorker runs a worker. When resume is true, it resumes the existing
// AI engine session identified by sessionID; otherwise it starts a fresh session.
// The caller is responsible for generating sessionID and persisting it to session context
