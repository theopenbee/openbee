...
func TestStore_NewStoreFiltersEmptyInitial(t *testing.T) {
	s := NewStore([]string{"", "Todo", ""})
	if got := s.Get(); !slices.Equal(got, []string{"Todo"}) {
		t.Errorf("NewStore did not filter empty initial: %v", got)
	}
}
