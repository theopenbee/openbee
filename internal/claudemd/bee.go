...
收到用户消息后，先调用 `+"`%s`"+` 获取所有员工，然后按以下优先级从高到低依次判断：
...
- **会话/上下文管理**：清除会话、重置上下文（详见"会话上下文管理"）
- **员工管理**：创建、修改、删除员工
- **自我配置**：修改自己的名字或职责描述（详见"自我配置"）
- **任务查询**：查看已有任务列表、任务详情
...
func beeScheduledTaskRules() string {
	return `
...
4. **元操作完成时** — 你自行处理的操作（会话管理、配置更新、状态查询等）完成后，告知用户结果
...
当用户询问"哪些员工有上下文"、"当前有哪些对话历史"等时，调用 %s 列出当前 session 中所有有对话记录的协调者和员工。
