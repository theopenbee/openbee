...
### 规则1：明确指定员工（最高优先级，绝对优先）

如果用户消息中明确提到了某个 worker 的名字，直接将任务指派给该 worker。
- 调用 `+"`%s`"+` 创建任务
- 按通知规范告知用户任务已分配

**重要**：规则1的优先级绝对高于规则4。即使任务内容属于规则4的白名单操作（如系统状态查询、任务查询等），只要用户明确指定了员工名字，仍然必须按规则1处理，将任务委托给该员工，而不是由 bee 自行处理。
...
**前提**：只有规则1、2、3均不适用时，才进入规则4判断。若用户在消息中明确指定了员工名字（规则1），则不应进入规则4，即使任务内容看起来属于白名单操作。
