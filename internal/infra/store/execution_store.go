...
func (s *ExecutionStore) Create(workerID, triggerInput, sessionID, accountName, engine string) (model.WorkerExecution, error) {
	...
	_, err := s.db.Exec(
		`INSERT INTO bee_executions (id, worker_id, session_id, account_name, engine, trigger_input, status, result, ai_process_pid, started_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, '', 0, ?)`,
		exec.ID, exec.WorkerID, exec.SessionID, exec.AccountName, exec.Engine, exec.TriggerInput, exec.Status, millis,
	)
	if err != nil {
...
func (s *ExecutionStore) CreateBeeExecution(sessionID, triggerInput, engine string) (model.WorkerExecution, error) {
	...
	_, err := s.db.Exec(
		`INSERT INTO bee_executions (id, worker_id, session_id, account_name, engine, trigger_input, status, result, ai_process_pid, started_at)
		 VALUES (?, NULL, ?, ?, ?, ?, ?, '', 0, ?)`,
		exec.ID, exec.SessionID, exec.AccountName, exec.Engine, exec.TriggerInput, exec.Status, millis,
	)
	if err != nil {
...
const execSelect = `
SELECT e.id, e.worker_id, e.session_id, e.account_name, e.engine, e.trigger_input, e.status, e.result, e.log_path,
...
func scanExecution(scanner interface{ Scan(...any) error }) (model.WorkerExecution, error) {
	...
	err := scanner.Scan(&e.ID, &e.WorkerID, &e.SessionID, &e.AccountName, &e.Engine, &e.TriggerInput, &e.Status, &e.Result, &e.LogPath, &e.AIProcessPID, &e.StartedAt, &e.CompletedAt, &e.WorkerName)
	return e, err
}
...
func executionFilterWhere(f ExecutionFilter) (string, []any) {
	...
	if f.AccountName != "" {
		b.add("e.account_name = ?", f.AccountName)
	}
	if f.Status != "" {
