...
func TestSeenSet_AddEmptySliceIsNoop(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "seen.ndjson")); !os.IsNotExist(err) {
...
func TestSeenSet_AddCreatesDir(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "seen.ndjson")); err != nil {
		t.Errorf("seen.ndjson not written: %v", err)
	}
}
