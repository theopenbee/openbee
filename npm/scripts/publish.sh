...
publish_package() {
  ...
  echo "Publishing @openbee/$pkg_name..."

  local output
  output=$(npm publish "$pkg_dir" --access public 2>&1) && {
    echo "$output"
    echo "Successfully published @openbee/$pkg_name"
    return 0
  }

  echo "$output"
  if echo "$output" | grep -q "You cannot publish over the previously published versions"; then
    echo "Package @openbee/$pkg_name already published (idempotent, treating as success)"
  else
    echo "Error publishing @openbee/$pkg_name"
    exit 1
  fi
}
